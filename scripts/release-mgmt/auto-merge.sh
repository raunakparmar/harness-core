...
echo "STEP 4: Checking if Master branch is identical to Develop branch"
MASTER_TO_DEVELOP=$(git log --graph --abbrev-commit --date=relative develop..master)
if [ ! -z "$MASTER_TO_DEVELOP" ]; then
    echo "ERROR: Line $LINENO: Master and Develop branches are not identical. Exiting..."
    exit 1
fi
...
MASTER_TO_DEVELOP=$(git log --graph --abbrev-commit --date=relative develop..master)
DEVELOP_TO_MASTER=$(git log --graph --abbrev-commit --date=relative master..develop)
